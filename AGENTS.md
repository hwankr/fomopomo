...
<omx_selection_policy>
Default execution policy for this project:
- Choose the lightest adequate path automatically; the user should not need to name OMX explicitly.
- Work directly for small, local, low-overhead tasks: quick answers, single-file edits, short bug fixes, focused tests, and narrow refactors.
- Use OMX CLI surfaces when work benefits from orchestration, resumability, persistent mode state, tmux workers, or multi-stage monitoring.
- Prefer `omx team` for broad multi-file work, parallelizable refactors, or tasks that can be split into independent lanes.
- Prefer `omx ralph` for long-running or resumable work that should preserve PRD/progress state under `.omx/`.
- Prefer `omx doctor` / `omx doctor --team` for installation, runtime, tmux, or team diagnostics.
- Prefer `omx status`, `omx hud`, `omx cancel`, and `omx team status|resume|shutdown` for mode and lifecycle management after OMX has been engaged.
- Use `omx hooks` or `omx tmux-hook` only when the task is explicitly about hook plugins, tmux prompt injection, or event-driven automation.
- If OMX overhead is not justified, do not force it.
- When OMX is used, briefly state which OMX surface is being used and why.
</omx_selection_policy>

...
export default function MemberCard({
    ...
    return (
        <div
            ...
            className={`bg-white dark:bg-slate-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm border transition-all cursor-pointer hover:border-rose-200 dark:hover:border-rose-800 gap-4 ${isStudying
        ...
        >
            <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4 w-full sm:w-auto">
                <div className="flex items-center justify-center w-6 sm:w-8 flex-shrink-0">
                    {rankBadge}
                </div>
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-lg font-bold text-white flex-shrink-0 ${isStudying ? 'bg-rose-500 animate-pulse' : isPaused ? 'bg-amber-500' : 'bg-gray-300 dark:bg-slate-600'
                    }`}>
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                        {isCurrentUser && editingNickname ? (
                            <div className="flex min-w-0 w-full max-w-64 items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                <input
                                    type="text"
                                    value={tempNickname}
                                    onChange={(e) => onTempNicknameChange(e.target.value)}
                                    className="min-w-0 w-full flex-1 px-2 py-1 text-sm border rounded dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                                    aria-label="내 닉네임"
                                    placeholder="닉네임"
                                    autoFocus
                                />
                                <button onClick={onSaveNickname} aria-label="닉네임 저장" className="text-green-500 hover:text-green-600 flex-shrink-0">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
                                        <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                    </svg>
                                </button>
                                <button onClick={onCancelEditNickname} aria-label="닉네임 수정 취소" className="text-red-500 hover:text-red-600 flex-shrink-0">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
                            ...
                            <h3 className="min-w-0 max-w-full font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            <div className="flex min-w-0 flex-wrap items-center justify-end gap-3 w-full sm:w-auto sm:shrink-0">
